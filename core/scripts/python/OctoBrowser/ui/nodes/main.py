...
class nodes(QtWidgets.QGraphicsView):
    ...
    def __init__(self, parent, configPath=defaultConfigPath):
        ...
        """
        super(nodes, self).__init__(parent)
    ...
    def mousePressEvent(self, event):
        ...
        super(nodes, self).mousePressEvent(event)
    ...
    def mouseMoveEvent(self, event):
        ...
        super(nodes, self).mouseMoveEvent(event)
    ...
    def mouseReleaseEvent(self, event):
        ...
        super(nodes, self).mouseReleaseEvent(event)
    ...
    def loadConfig(self, filePath):
        """
        Set a specific configuration for this instance of nodes.
...
class NodeItem(QtWidgets.QGraphicsItem):
    ...
    def paint(self, painter, option, widget):
        ...
        for attr in self.attrs:
            nodesInst = self.scene().views()[0]
            config = nodesInst.config
            ...
            if nodesInst.drawingConnection:
                if self == nodesInst.currentHoveredNode:
                    if (attrData['dataType'] != nodesInst.sourceSlot.dataType or
                        (nodesInst.sourceSlot.slotType == 'plug' and attrData['socket'] == False or
                         nodesInst.sourceSlot.slotType == 'socket' and attrData['plug'] == False)):
    ...
    def hoverLeaveEvent(self, event):
        ...
        for item in nodesInst.scene().items():
            if isinstance(item, ConnectionItem):
                item.setZValue(0)
...
class SlotItem(QtWidgets.QGraphicsItem):
    ...
    def mousePressEvent(self, event):
        ...
        if event.button() == QtCore.Qt.LeftButton:
            ...
            nodesInst = self.scene().views()[0]
            nodesInst.drawingConnection = True
            nodesInst.sourceSlot = self
            nodesInst.currentDataType = self.dataType
        else:
                ...
                if self.parentItem() not in targets:
                    for target in targets:
                        if isinstance(target, NodeItem):
                            nodesInst.currentHoveredNode = target
            else:
                nodesInst.currentHoveredNode = None
    ...
    def mouseReleaseEvent(self, event):
        ...
        nodesInst = self.scene().views()[0]
        if event.button() == QtCore.Qt.LeftButton:
            nodesInst.drawingConnection = False
            nodesInst.currentDataType = None
    ...
    def paint(self, painter, option, widget):
        ...
        config = nodesInst.config
        if nodesInst.drawingConnection:
            if self.parentItem() == nodesInst.currentHoveredNode:
                painter.setBrush(utils._convertDataToColor(config['non_connectable_color']))
                if (self.slotType == nodesInst.sourceSlot.slotType or (self.slotType != nodesInst.sourceSlot.slotType and self.dataType != nodesInst.sourceSlot.dataType)):
                    painter.setBrush(utils._convertDataToColor(config['non_connectable_color']))
                else:
...
class PlugItem(SlotItem):
    ...
    def connect(self, socket_item, connection):
        ...
        # Emit signal.
        nodesInst = self.scene().views()[0]
        nodesInst.signal_PlugConnected.emit(connection.plugNode, connection.plugAttr, connection.socketNode, connection.socketAttr)
    ...
    def disconnect(self, connection):
        ...
        # Emit signal.
        nodesInst = self.scene().views()[0]
        nodesInst.signal_PlugDisconnected.emit(connection.plugNode, connection.plugAttr, connection.socketNode, connection.socketAttr)
...
class SocketItem(SlotItem):
    ...
    def connect(self, plug_item, connection):
        ...
        # Emit signal.
        nodesInst = self.scene().views()[0]
        nodesInst.signal_SocketConnected.emit(connection.plugNode, connection.plugAttr, connection.socketNode, connection.socketAttr)
    ...
    def disconnect(self, connection):
        ...
        # Emit signal.
        nodesInst = self.scene().views()[0]
        nodesInst.signal_SocketDisconnected.emit(connection.plugNode, connection.plugAttr, connection.socketNode, connection.socketAttr)
...
class ConnectionItem(QtWidgets.QGraphicsPathItem):
    ...
    def mousePressEvent(self, event):
        ...
        if d_to_target < d_to_source:
            ...
            self.target = None
            nodesInst.sourceSlot = self.source
        else:
            self.source_point = event.pos()
            self.movable_point = 'source_point'
            self.source.disconnect(self)
            self.source = None
            nodesInst.sourceSlot = self.target
    ...
    def mouseMoveEvent(self, event):
        ...
        if any(isinstance(target, NodeItem) for target in targets):
            ...
            if nodesInst.sourceSlot.parentItem() not in targets:
                for target in targets:
                    if isinstance(target, NodeItem):
                        nodesInst.currentHoveredNode = target
        else:
