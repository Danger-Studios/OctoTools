...
stickyProps = [
    ...
    [ 'usdloglevel', True ],
    [ 'usdpool', False ],
    [ 'usdsecondarypool', False ],
    [ 'usdgroup', False ],
    [ 'usdpriority', True ],
    [ 'usdtasktimeout', True ],
    [ 'usdautotimeout', True ],
    [ 'usdmachinelimit', True ],
    [ 'usdisblacklist', True ],
    [ 'usdmachinelist', False ],
    [ 'usdlimits', False ],
    [ 'usdconcurrent', True ],
    [ 'usdslavelimit', True ],
    [ 'usdlocalexport', True ],
    [ 'usdonjobcomplete', False ],
...
def InitializeDialog():
    ...
    dialog.setValue( "usdjob.val", 0 )
    dialog.setValue( "usdthreads.val", 0 )
    dialog.setValue( "usdloglevel.val", 0 )
    dialog.setMenuItems( "usdpool.val", pools )
    dialog.setMenuItems( "usdsecondarypool.val", secondaryPools )
    dialog.setMenuItems( "usdgroup.val", groups )
    dialog.setValue( "usdpriority.val", maxPriority / 2 )
    dialog.setValue( "usdtasktimeout.val", 0 )
    dialog.setValue( "usdautotimeout.val", 0 )
    dialog.setValue( "usdconcurrent.val", 1 )
    dialog.setValue( "usdslavelimit.val", 1 )
    dialog.setValue( "usdmachinelimit.val", 0 )
    dialog.setValue( "usdmachinelist.val", "" )
    dialog.setValue( "usdisblacklist.val", 0 )
    dialog.setValue( "usdlimits.val", "" )
    dialog.setValue( "usdlocalexport.val", 0 )
    dialog.setMenuItems( "usdonjobcomplete.val", ["Nothing", "Archive", "Delete"] )
...
def Callbacks():
    ...
    dialog.addCallback( "usdjob.val", UsdStandaloneCallback )
    dialog.addCallback( "usdloglevel.val", UsdLogLevelCallback )
    dialog.addCallback( "usdthreads.val", UsdThreadsCallback )

    dialog.addCallback( "usdgetmachinelist.val", UsdGetMachineListFromDeadline )
    dialog.addCallback( "usdgetlimits.val", UsdGetLimitGroupsFromDeadline )

    dialog.addCallback( "usdpriority.val", UsdJobPriorityCallback )
    dialog.addCallback( "usdtasktimeout.val", UsdTaskTimeoutCallback )
    dialog.addCallback( "usdconcurrent.val", UsdConcurrentTasksCallback )
    dialog.addCallback( "usdmachinelimit.val", UsdMachineLimitCallback )
...
def UsdStandaloneCallback():
    global dialog, valuesToToggle

    isUsdJob = dialog.value( "usdjob.val" )
    ToggleValues( valuesToToggle[ "usd" ], isUsdJob )

def UsdLogLevelCallback():
    ClampValue("usdloglevel.val", 0, 10)

def UsdThreadsCallback():
    ClampValue( "usdthreads.val", 0, 256 )

def UsdGetMachineListFromDeadline():
    GetDeadlineValues( "-selectmachinelist", "usdmachinelist.val" )

def UsdGetLimitGroupsFromDeadline():
    GetDeadlineValues( "-selectlimitgroups", "usdlimits.val" )

def UsdGetDependenciesFromDeadline():
    GetDeadlineValues( "-selectdependencies", "usddependencies.val" )

def UsdJobPriorityCallback():
    global maxPriority

    ClampValue( "usdpriority.val", 0, maxPriority )

def UsdTaskTimeoutCallback():
    ClampValue( "usdtasktimeout.val", 0, 1000000 )

def UsdConcurrentTasksCallback():
    ClampValue( "usdconcurrent.val", 1, 16 )

def UsdMachineLimitCallback():
    ClampValue( "usdmachinelimit.val", 0, 1000000 )
...
def GetJobProperties( dialog, groupBatch=None, jigsawRegionCount=None, jigsawRegions=None):
    ...
    return {
        ...
        "usdjob" : dialog.value( "usdjob.val" ),
        "usdpool" : dialog.value( "usdpool.val" ),
        "usdsecondarypool" : "" if dialog.value( "usdsecondarypool.val" ) == " " else dialog.value( "usdsecondarypool.val" ),
        "usdgroup" : dialog.value( "usdgroup.val" ),
        "usdpriority" : dialog.value( "usdpriority.val" ),
        "usdtasktimeout" : dialog.value( "usdtasktimeout.val" ),
        "usdautotimeout" : dialog.value( "usdautotimeout.val" ),
        "usdconcurrent" : dialog.value( "usdconcurrent.val" ),
        "usdmachinelimit" : dialog.value( "usdmachinelimit.val" ),
        "usdslavelimit" : dialog.value( "usdslavelimit.val" ),
        "usdlimits" : dialog.value( "usdlimits.val" ),
        "usdonjobcomplete" : dialog.value( "usdonjobcomplete.val" ),
        "usdisblacklist" : dialog.value( "usdisblacklist.val" ),
        "usdmachinelist" : dialog.value( "usdmachinelist.val" ),
        "usdthreads" : dialog.value( "usdthreads.val" ),
        "usdloglevel" : dialog.value( "usdloglevel.val" ),
        "usdlocalexport" : dialog.value( "usdlocalexport.val" ),
...
def openIntegrationWindow():
    ...
    integrationOptions = ["-ExecuteScript", integrationPath, "-v", "2", "-d", "Houdini", "Draft", "Shotgun", "FTrack", "--path", scenePath]
...
def SubmitJobCallback():
    ...
    for node in jobs:
        ...
        if ifdPath != None:
            ...
            if dialog.value( "usdjob.val" ) == 1 and dialog.value( "usdlocalexport.val" ) != 1 and isUSD:
                totalJobs += 1
    ...
    if missingIFDPaths != "" and dialog.value( "usdjob.val" ):
        warningMessage.append( "The Dependent USD Standalone job option is enabled, but the following ROPs don't have the Disk File option enabled to export IFD files: \n\n%s\n" % missingIFDPaths )
