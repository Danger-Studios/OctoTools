...
def CallDeadlineCommand( arguments, hideWindow=True, readStdout=True ):
    ...
    if six.PY2:
        ...
        proc = subprocess.Popen(
            ...
            startupinfo=startupinfo, creationflags=creationflags, encoding="utf-8")
    output = ""
    if readStdout:
...
def GetOutputPath( node ):
    ...
    if nodeType == "Geometry" or nodeType == "Filmbox FBX" or ( nodeType == "ROP Output Driver" and category == "Sop" ):
    ...
    elif nodeType == "USD" or nodeType == "HuskStandalone":
        ...
        if driverNode:
            ...
            # if no driver is set, use one connected to node input
            driverNode = node.input(0)
            if driverNode:
                outputFile = GetOutputPath(driverNode)
    ...
    if outputFile and \
       type(outputFile) == hou.Parm and \
       not os.path.isabs( outputFile.eval() ):
        outputFile = "COMMAND"
...
def isExportJob( node, jobProperties ):
    ...
    if nodeType == "Mantra":
        ...
        return jobProperties.get("usdjob", False)
    elif nodeType == "Arnold":
...
def isExportLocal( node, jobProperties ):
    ...
    if isExportJob( node, jobProperties ):
        ...
        if nodeType == "Mantra":
            ...
            return jobProperties.get("usdlocalexport", False)
        elif nodeType == "Arnold":
...
def write_asset_paths_to_job_file(asset_paths, job_file):
    ...
    for index, asset_path in enumerate(asset_paths):
        job_file.write("AWSAssetFile{0}={1}\n".format(index, asset_path.replace("\\", "/")))
...
def SubmitRenderJob( node, jobProperties, dependencies ):
    ...
    for wedgeNum in range(wedgeJobCount):
        # set wedge num before expanding the paths
        ...
        if localExport:
            ...
            for regionjobNum in range( 0, regionJobCount ):
                ...
                with io.open( jobInfoFile, "w" , encoding="utf-8") as fileHandle:
                    ...
                    if not exportJob:
                        if paddedOutputFile != "":
                            ...
                            splitOutput = os.path.split(outputFile)
                            fileHandle.write("OutputDirectory0={}\n".format(splitOutput[0]))
                            fileHandle.write("OutputFilename0={}\n".format(splitOutput[1]))
                    elif ifdFile != "":
                ...
                with io.open( pluginInfoFile, "w" , encoding="utf-8") as fileHandle:
        ...
        if exportJob:
            ...
            for exportJobNum in range( 0, exportJobCount ):
                ...
                with io.open( exportJobInfoFile, 'w' , encoding="utf-8") as fileHandle:
                    ...
                    if paddedOutputFile != "":
                        ...
                        if not os.path.exists(os.path.dirname(paddedOutputFile)):
                            os.makedirs(os.path.dirname(paddedOutputFile))

                if not tilesEnabled:
                    ConcatenatePipelineToolSettingsToJob( exportJobInfoFile, jobProperties.get( "jobname", "Untitled" ) )

                with io.open( exportPluginInfoFile, 'w' , encoding="utf-8") as fileHandle:
                    ...
                    elif exportType == "HuskStandalone":
                        fileHandle.write("SceneFile={}\n".format(ifdFile))
                        fileHandle.write("OutputPath={}\n".format(paddedOutputFile.replace("####", "%04d")))
                        fileHandle.write("LogLevel={}\n".format(jobProperties.get("usdloglevel", 0)))
        ...
        if tilesEnabled and jobProperties.get( "submitdependentassembly" ) and ( renderJobIds or exportJobIds ):
            ...
            for frame in renderFrames:
                ...
                with io.open( configFilename, "w" , encoding="utf-8") as fileHandle:
                    fileHandle.write( "\n" )
